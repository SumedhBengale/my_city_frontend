...
function ContactUs() {
  //List of checkboxes
  const checkboxes = ['Email', 'Phone', 'Whatsapp', 'Text Message'];
  ...
  const handleSubmit = () => {
    ...
    if (
      ...
      addContactUsRequest({
        ...
        name, email, phone, message, selectedcheckboxes}).then((res) => {
        console.log(res)
        if(res.status === 201){
      ...
      //show toast error
      toast.error('Please provide your name and email or phone number and at least one contact method')
    }
  };
    ...
    <>
      ...
      > {/* Background Image */}
        ...
        <div className='flex justify-center items-center mx-10'>
            <img src={whiteLogo} alt='My City Logo' className='w-48 lg:w-72 self-start mb-10'></img>
          </div>
            <div className='font-custom-bold text-4xl md:text-5xl text-white text-center pb-4 capitalize'>Contact Us</div>
      ...
      <FadeInSetion>
          <div className='w-full h-full px-5 bg-white -translate-y-24 rounded-tl-[50px] md:rounded-tl-[100px] pt-20'>
            <div className='flex flex-col justify-center items-start h-full bg-gray-200 w-full md:px-5 m-5 rounded-lg container mx-auto'>
              <div className='font-custom-bold text-xl mx-3 md:mx-0 mt-10 text-black'>Get In Touch</div>
              <hr className="w-36 border-sm my-4 ml-3 mx-3 md:mx-0 border-[1px] border-black" />
                ...
                <div className='w-full grid grid-cols-1 lg:grid-cols-2 place-items-start'>
                    <div className='w-full flex flex-col justify-center items-start order-2 lg:order-1'>
                      <div className='font-custom text-lg mx-3 md:mx-0 mt-3 text-black'>Name</div>
                      <input className='w-full h-10 rounded-lg px-3 mx-3 md:mx-0 drop-shadow-md' type='text' value={name} onChange={(e)=>setName(e.target.value)}></input>

                      <div className='font-custom text-xl mx-3 md:mx-0 mt-3 text-black'>Phone</div>
                      <input className='w-full h-10 rounded-lg px-3 mx-3 md:mx-0 drop-shadow-md' type='phone' value={phone} onChange={(e)=>{
                        //Only allow numbers
                        const re = /^[0-9\b]+$/;
                      ...
                      <div className='font-custom text-xl mx-3 md:mx-0 mt-3 text-black'>Email</div>
                      <input className='w-full h-10 rounded-lg px-3 mx-3 md:mx-0 drop-shadow-md' type='email' value={email} onChange={(e)=>setEmail(e.target.value)}></input>

                      <div className='font-custom text-xl mx-3 md:mx-0 mt-3 text-black'>Message</div>
                      <textarea className='w-full h-40 rounded-lg px-3 mx-3 md:mx-0 drop-shadow-md' type='text' value={message} onChange={(e)=>setMessage(e.target.value)}></textarea>

                      <div className='font-custom-bold text-xl mx-3 md:mx-0 mt-3 text-black'>Prefferred method of contact</div>
                      <div className='flex flex-row justify-start items-center mx-3 md:mx-0'>
                        <div className='grid grid-cols-2 gap-2'>

                        {checkboxes.map((checkbox) => (
              <div key={checkbox} className='flex items-center gap-1 mt-1'>
                <input
                  type='checkbox'
                  id={checkbox}
                  name={checkbox}
                ...
                <label
                  htmlFor={checkbox}
                  className={` h-5 w-5 border border-secondary rounded-md cursor-pointer transition-colors ${selectedcheckboxes.includes(checkbox)? 'bg-secondary' : 'bg-white'}`}
                >
                  {
                ...
                <span className='text-sm pl-2 overflow-hidden overflow-ellipsis'>{checkbox}</span>
              </div>
            ))}
                        </div>
                      </div>
                  ...
                  <div className='px-10 w-full drop-shadow-md rounded-md order-1 lg:order-2'>
                    <MapContainer coordinate={{lat: '40.7128',lng: '-74.0060'}} />
                </div>                
              </div>

              <div className='flex flex-col justify-center items-center w-full'>
                <button className='w-40 h-10 bg-primary hover:bg-secondary text-white rounded-lg mt-5 mb-5' onClick={()=>{handleSubmit()}}>Submit</button>
              </div>
