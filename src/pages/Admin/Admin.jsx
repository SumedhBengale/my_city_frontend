...
function Admin() {
  ...
  useEffect(() => {
    ...
    try {
      const checkAdmin = async () => {
        ...
        await axios
          .post(
            `${config.API_URL}/admin/checkAdmin`,
            {
  ...
  const sidebarElements = [
    ...
    { id: 2, label: "Chats" },
    { id: 3, label: "Reviews" },
    { id: 4, label: "Contact Requests" },
    { id: 5, label: "Enquiries" },
  ...
  return checkComplete ? (
    <div className="flex relative flex-col h-screen">
      ...
      <div className="md:hidden z-30 sticky w-full">{<NavbarBlack />}</div>
      {/* Sidebar */}
      <div className="flex flex-col sm:flex-row h-full w-full">
        <div className="w-full sm:w-1/2 md:w-1/4 h-min sm:h-full bg-[#262150] transition duration-75 p-4">
          <div className="flex justify-between items-center">
            <div className="text-lg font-bold sm:mb-4 text-center text-white">
              Admin Panel
            </div>
        ...
        <div className="w-full p-4">
          {selectedElement.label === "Users" ? (
            <SearchComponent searchType={"user"} />
          ) : selectedElement.label === "Chats" ? (
            <SearchComponent searchType={"chat"} />
          ) : selectedElement.label === "Reviews" ? (
            <SearchComponent searchType={"review"} />
          ) : selectedElement.label === "Contact Requests" ? (
            <ContactRequests />
          ) : selectedElement.label === "Enquiries" ? (
            <Enquiries></Enquiries>
          ) : null}
        </div>
      </div>
