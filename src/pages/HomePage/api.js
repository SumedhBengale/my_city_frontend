...
export const getFeaturedResidences = async () => {
    try {
        //get request with auth header
        const listingsData = await axios.get(`${config.STRAPI_URL}/api/featured-listing`, {
        });
...
export const getHomepageImages = async () => {
    try{
        const response = await axios.get(`${config.STRAPI_URL}/api/images?filters[name][$eq]=HomePageImages&populate=*`, {
        })
        return response.data;
    }
    catch(error){
