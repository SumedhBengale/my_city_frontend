    ...
    <div className='w-full flex justify-center'>
      <div className='w-full md:max-w-[800px] flex flex-col'>
        <div className='w-full flex justify-center mb-5'>
            <h1 className='text-2xl font-custom text-center text-primary px-2'>Hear From Our Happy Customers</h1>
        </div>
          <div className='flex justify-between'>
            <button className='bg-white w-full text-white z-20'
            ...
            {reviews ? reviews.map((review) => (
                <div key={review.id} className="review-card text-center">
                    ...
                    {Array.from(Array(5 - review.attributes.rating).keys()).map((index) => {
                    return (
                        <svg
                        key={index}
                  ...
                  <div className='w-full flex flex-col items-center'>
                      <div className=" w-5/6 text-center text-lg text-primary font-custom">{review.attributes.review}</div>
                      <div className='h-20 flex justify-center w-full'>
                        <img src={`${config.STRAPI_URL}`+review.attributes.profile_photo.data.attributes.url} alt='Profile' className="rounded-full w-20" />
                      </div>                
